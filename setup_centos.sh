#!/bin/sh
#    Setup Simple IPSec/L2TP VPN server for CentOS
#
#    Dennis Anfossi <danfossi@itfor.it> Copyright (C) 2015 
#	    Porting of work of Phil Plückthun <phil@plckthn.me> on CentOS
...
then
  echo "This script was designed to run on CentOS!"
...
echo "Making sure that epel repos is installed.."
...
yum install unbound nss libcap-ng curl bison flex xmlto -y > /dev/null
...
# Download and install Libreswan
echo "Installing LibreSwan.."
rpm -qa | grep libreswan
if [ "$?" = "1" ]
then
  yum install libreswan -y > /dev/null
fi
